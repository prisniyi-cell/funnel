...
return res.send(`<html><body style="font-family:sans-serif;padding:20px;background:#f3f4f6"> <div style="max-width:400px;margin:80px auto;background:#fff;padding:32px;border-radius:12px;box-shadow:0 2px 8px rgba(0,0,0,0.1)"> <h2 style="margin:0 0 20px">💰 Dollar Skill Admin</h2> <form action="/admin" method="get"> <input type="password" name="pass" placeholder="Password" style="width:100%;padding:10px;font-size:16px;border:1px solid #ddd;border-radius:6px;box-sizing:border-box;margin-bottom:12px"/> <button type="submit" style="width:100%;padding:10px;background:#25d366;color:white;border:none;border-radius:6px;font-size:16px;cursor:pointer">Login</button> </form> </div></body></html>`);
}
...
html += `<div class="cooled-header" onclick="toggleCooled()"> <div class="section-title" style="margin:0">Cooled Off (${cooledPhones.length})${cooledUnread > 0 ? ` <span style="background:#87ceeb;color:#fff;border-radius:20px;padding:2px 8px;font-size:11px;margin-left:6px">${cooledUnread} new</span>` : ‘’}</div>
